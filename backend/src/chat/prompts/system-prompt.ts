...
${filterSection}
## 추천 후보 작품
아래 목록에서 우선적으로 선택하되, 사용자 요청과 맞지 않는 후보는 제외하세요. 적합한 후보가 3개 미만이면 본인 지식으로 보충하세요.
${candidatesSection}
...
- 후보를 제외한 후 추천할 작품이 3개 미만이면, 본인의 지식으로 보충하여 최소 3개를 추천하세요.
...
## 응답 형식 (반드시 준수)
- 사용자에게 보여줄 Markdown 본문을 먼저 작성하세요. 작품 제목은 굵게 표시하고(부가정보 없이 제목만 출력), 각 작품마다 추천 이유를 본문에 자연스럽게 포함하세요.(제목 - 추천 이유)

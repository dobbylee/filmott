...
## 추천 출력 형식 (필수)
작품을 추천할 때 응답 마지막에 반드시 아래 JSON 블록을 추가하세요. 이것은 시스템이 추천 카드를 생성하는 데 사용됩니다.
---RECOMMENDATIONS---
[{"tmdbId": 숫자, "contentType": "movie 또는 tv", "title": "작품 제목"}]
---END---
- tmdbId: 참고 목록의 작품은 목록의 ID를 사용하세요. 목록에 없는 작품은 본인이 아는 TMDB ID를 사용하거나, 모르면 0으로 적으세요.
- title: 한국어 제목을 반드시 포함하세요.
- 이 블록은 사용자에게 보이지 않습니다. 작품을 추천했다면 반드시 포함하세요.
- 작품 추천 없이 일상 대화만 한 경우에만 이 블록을 생략하세요.`;
}

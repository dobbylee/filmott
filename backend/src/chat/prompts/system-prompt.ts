...
## 응답 형식
- 추천 작품명은 반드시 **한국어 제목 (영어 원제)** 형식으로 볼드 표기하세요. 예: **기생충 (Parasite)**, **오징어 게임 (Squid Game)**, **원스 (Once)**
- 영어 원제가 같거나 없는 경우에도 괄호 안에 영어 제목을 반드시 넣으세요.
- 자연스러운 대화체로 각 작품의 추천 이유를 1~2문장으로 설명하세요.
- 볼드로 표기된 제목은 시스템이 자동으로 추천 카드를 생성합니다.`;
}

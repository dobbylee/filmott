...
async function main() {
  ...
  console.log(
    `=== metadata 재캐싱 시작 (${rows.length}건, 동시 ${CONCURRENCY}개) ===\n`,
  );
  ...
  for (let i = 0; i < rows.length; i += CONCURRENCY) {
    ...
    const results = await Promise.allSettled(
      batch.map((row) =>
        embeddingService.cacheContentMetadata(row.content_id, true),
      ),
    );
    ...
    if (processed % 50 < CONCURRENCY || processed === rows.length) {
      ...
      console.log(
        `  ${processed}/${rows.length} (캐싱=${cached}, 실패=${failed}) [${elapsed}초]`,
      );
    }

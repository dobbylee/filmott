...
describe('ContentDetailTracker', () => {
  ...
  it('마운트 시 content_detail_view 이벤트를 호출해야 한다', () => {
    render(
      <ContentDetailTracker tmdbId="123" contentType="movie" />,
    );
  ...
  it('아무것도 렌더링하지 않아야 한다 (null 반환)', () => {
    const { container } = render(
      <ContentDetailTracker tmdbId="1" contentType="movie" />,
    );
  ...
  it('tmdbId가 변경되면 이벤트를 다시 호출해야 한다', () => {
    const { rerender } = render(
      <ContentDetailTracker tmdbId="100" contentType="movie" />,
    );

    expect(mockTrackEvent).toHaveBeenCalledTimes(1);

    rerender(
      <ContentDetailTracker tmdbId="200" contentType="movie" />,
    );
